...
import {
  ...
  getPfrStep,
  getRecommendation,
  postSection9Recommendation,
  updateSection9Recommendation,
...
const AddPlanRecommendation = () => {
  ...
  const [dataCISPremiumType, setCISDataPremiumType] = useState<any>([
    { id: 4, name: "Single Payment" },
  ]);
  const [dataPremiumType, setDataPremiumType] = useState<any>([
    { id: 0, name: "CASH" },
    { id: 1, name: "CPF OA" },
    { id: 2, name: "CPF SA" },
  ...
  const [dataPaymentFreq, setDataPaymentFreq] = useState<any>([
    { id: 0, name: "Monthly" },
    { id: 1, name: "Quarterly" },
  ...
  const [dataSelectedCategoryType, setDataSelectedCategoryType] =
    useState<any>(-1);
  ...
  const onEditCis = (resData: any, companyIds: any, cisIds: any) => {
    ...
    if (resData.cis.length > 0) {
      resData.cis.map((dataCis: any, indexCis: any) => {
        //
        if (
          (dataOutcomes[section9Recommend.product.nameOfOwner] == 1 ||
            dataCis.platform.mustPassCKA == 0) &&
          companyIds == dataCis.platform.companyId &&
          ((dataCis.maxBudget == 0 &&
            singlePayorBudget[section9Recommend.product.nameOfOwner][
              dataCis.payment
            ] <= 29999) ||
            (dataCis.maxBudget == 1 &&
              singlePayorBudget[section9Recommend.product.nameOfOwner][
  ...
  const onEditProvider = (resInitWhole: any, resCateg: any, resComp: any) => {
    ...
    const selectedCategoryType = resInitWhole.category[resCateg]
      ? resInitWhole.category[resCateg]["type"]
      : null;
    setDataSelectedCategoryType(selectedCategoryType);
    const selectedCategoryId = resInitWhole.category[resCateg]
      ? resInitWhole.category[resCateg]["id"]
      : null;
    ...
    var products = resInitWhole.company[getSelectedCompany]
      ? resInitWhole.company[getSelectedCompany]["products"]
      : null;
      ...
      if (products) {
        products.map((product: any, k: any) => {
          if (
            ...
            product["ilp"] != null &&
            product["ilp"]["platform"]["fundType"] ==
              section9Recommend.product.premiumType &&
            product["ilp"]["riskCategory"] ==
              resInitWhole.section5Result[section9Recommend.product.nameOfOwner]
          ) {
    ...
    if (selectedCategoryType == 1) {
      var modelPortfolioRiskCategory = Number(
        resInitWhole.section5Result[section9Recommend.product.nameOfOwner]
      );
      setProduct(
        modelPortfolioRiskCategory.toString(),
        "modelPortfolioRiskCategory",
        null
      );
      const ilpFundsOfCompany = resInitWhole.ilpFunds.filter(
        (fund: any, k: any) => {
          if (resInitWhole.company[getSelectedCompany]) {
            return (
              fund["companyId"] ==
              resInitWhole.company[getSelectedCompany]["idReal"]
            );
          }
        }
      );
    ...
    const selectedCategoryType1 = resInitWhole.category[getSelectedCategory]
      ? resInitWhole.category[getSelectedCategory]["type"]
      : null;
    setDataSelectedCategoryType(selectedCategoryType);

    const selectedCategoryId1 = resInitWhole.category[getSelectedCategory]
      ? resInitWhole.category[getSelectedCategory]["id"]
      : null;
    var products = resInitWhole.company[resComp]
      ? resInitWhole.company[resComp]["products"]
      : null;
    if (selectedCategoryType == 1) {
      // IF ILP
      if (section9Recommend.product.premiumType == -1) {
  ...
  const changeDataCategory = (event: any) => {
    ...
    const selectedCategoryType = initWhole.category[value]
      ? initWhole.category[value]["type"]
      : null;
    setDataSelectedCategoryType(selectedCategoryType);
    const selectedCategoryId = initWhole.category[value]
      ? initWhole.category[value]["id"]
      : null;
    setDataSelectedCategoryId(selectedCategoryId);
    // Check Product Company
    var pushComp: Array<any> = [];
    ...
    var products = initWhole.company[getSelectedCompany]
      ? initWhole.company[getSelectedCompany]["products"]
      : null;
    if (selectedCategoryType == 1) {
      // IF ILP
      if (section9Recommend.product.premiumType == -1) {
        ...
        products.map((product: any, k: any) => {
          if (
            ...
            product["ilp"]["platform"]["fundType"] ==
              section9Recommend.product.premiumType &&
            product["ilp"]["riskCategory"] ==
              initWhole.section5Result[section9Recommend.product.nameOfOwner]
          ) {
            selectedProducts.push(product);
          }
        });
    ...
    if (selectedCategoryType == 1) {
      ...
      const ilpFundsOfCompany = initWhole.ilpFunds.filter(
        (fund: any, k: any) => {
          if (initWhole.company[getSelectedCompany]) {
            return (
              fund["companyId"] ==
              initWhole.company[getSelectedCompany]["idReal"]
            );
          }
        }
      );
  ...
  const changeDataProvider = (event: any) => {
    ...
    const selectedCategoryType = initWhole.category[getSelectedCategory]
      ? initWhole.category[getSelectedCategory]["type"]
      : null;
    setDataSelectedCategoryType(selectedCategoryType);

    const selectedCategoryId = initWhole.category[getSelectedCategory]
      ? initWhole.category[getSelectedCategory]["id"]
      : null;
    var products = initWhole.company[value]
      ? initWhole.company[value]["products"]
      : null;
    if (selectedCategoryType == 1) {
      // IF ILP
      if (section9Recommend.product.premiumType == -1) {
    ...
    productFindOne(params).then((data) => {
      ...
      if (data.product.name) {
        ...
        if (
          ...
          section9Recommend.product.premiumType == 1
        ) {
        ...
        if (
          dataSelectedCategoryType != 1 ||
          section9Recommend.product.premiumType == 0
        ) {
        ...
        if (getSelectProductone["ilp"]) {
          if (getSelectProductone["ilp"].platform.funds.length > 0) {
            getSelectProductone["ilp"].platform.funds.map(
              (valueRes: any, indexRes: any) => {
                dataFunds.push({
  ...
  const handleBenefits = (event: any, index: any) => {
    ...
    if (dataBenVal >= 0) {
      ...
      section9Recommend.product.benefit.map((resBen: any, resIBen: any) => {
        if (resBen.benefitId != value) {
          resBenefit.push({
            benefitId: resBen.benefitId,
            content: resBen.content,
            title: resBen.title,
          });
        }
      });
      ...
      dataProductSelected.benefits.map((valueBenefit: any, index: any) => {
        if (valueBenefit.id == value) {
          resBenefit.push({
            benefitId: value,
            content: valueBenefit.content,
            title: valueBenefit.title,
          });
        }
      });
      setProductArr(resBenefit, "benefit", null);
    ...
    if (resIndexRiskVal >= 0) {
      ...
      section9Recommend.product.risk.map((resProdRisk: any, resIRisk: any) => {
        if (resProdRisk.riskId != value) {
          resRisk.push({
            riskId: resProdRisk.riskId,
            content: resProdRisk.content,
            title: resProdRisk.title,
          });
        }
      });
        ...
        dataProductSelected.riders.map((valueProd: any, indexProd: any) => {
          if (valueProd.new_rider.id == riderId) {
            if (valueProd.new_rider.benefit.length > 0) {
              valueProd.new_rider.benefit.map(
                (valueBen: any, indexBen: any) => {
        ...
        dataProductSelected.riders.map((valueProd: any, indexProd: any) => {
          if (valueProd.new_rider.id == riderId) {
            if (valueProd.new_rider.risk.length > 0) {
              valueProd.new_rider.risk.map(
                (valueRisk: any, resIndexRiskVal: any) => {
  ...
  const saveData = async (params: any) => {
    ...
    try {
      ...
      if (s9_recommendId) {
        ...
        let storeData = await postSection9Recommendation(
          JSON.stringify(section9Recommend)
        );
        if (storeData.status == 200) {
          resetRecommendationProduct();
                  ...
                  <div>
                    <div className={`w-full my-4 space-y-3`}>
                      ...
                      <select
                        ...
                        value={section9Recommend.product.nameOfInsure}
                        name="nameOfInsure"
                  ...
                  <div>
                    {section9Recommend.product.nameOfInsure == "OTHER" ? (
                      <Input
                        label="Other"
                        className="my-4"
                        name="nameOfInsureOther"
                        value={section9Recommend.product.nameOfInsureOther}
                        handleChange={(event) => setProductData(event)}
                      />
                    ) : (
                      ""
                ...
                <RowDoubleGrid>
                  <div>
                    <Input
                      ...
                      className=""
                      name="premium"
                      value={section9Recommend.product.premium}
                      handleChange={(event) => setProductData(event)}
                      needValidation={true}
                      logic={
                        section9Recommend.product.premium === null ||
                        section9Recommend.product.premium === 0
                          ? false
                          : true
                      }
                    />
                  ...
                  <div>
                    <Input
                      ...
                      className=""
                      name="sumAssured"
                      value={section9Recommend.product.sumAssured}
                      handleChange={(event) => setProductData(event)}
                      needValidation={true}
                      logic={
                        section9Recommend.product.sumAssured === "" ||
                        section9Recommend.product.sumAssured === "-"
                          ? false
                          : true
                      }
                ...
                <RowDoubleGrid>
                  ...
                  <div>
                    <Input
                      ...
                      className=""
                      name="policyTerm"
                      value={section9Recommend.product.policyTerm}
                      handleChange={(event) => setProductData(event)}
                      needValidation={true}
                      logic={
                        section9Recommend.product.policyTerm === "" ||
                        section9Recommend.product.policyTerm === "-"
                          ? false
                          : true
                      }
                        ...
                        <table className="w-full text-sm divide-y rounded-md divide-gray-soft-strong">
                          ...
                          <tbody>
                            {getSelectProductone?.ilp &&
                              getSelectProductone.ilp.platform.funds.map(
                                (dataPlatform: any, indexPlatform: any) => (
                                  <tr key={"asada" + indexPlatform}>
                                    <td className="px-2 py-2">
                                      <span>
                                        {getFundName(dataPlatform.fund.groupId)}
                                      </span>
                                    </td>
                                    <td className="px-2 py-2">
                                    ...
                                    <td className="px-2 py-2">
                                      <select
                                        ...
                                        onChange={(event) =>
                                          setProductFund(
                                            ...
                                            dataPlatform.fund.groupId,
                                            indexPlatform
                                          )
                                        }
                                        disabled={
                                          getSelectProductone.ilp.platform
                                            .fixed == 1
                                            ? true
                                            : false
                                        }
                                      >
                                        ...
                                        {checkDataIlpFundsOfCompany(
                                          dataPlatform.fund.groupId
                                        ).length &&
                                          checkDataIlpFundsOfCompany(
                                            dataPlatform.fund.groupId
                                          ).map((val: any, indexVal: any) => (
                                            <option
                                              key={indexVal}
                                              value={val.id}
                                            >
                                ...
                                <Input
                                  ...
                                  value={
                                    section9Recommend.product
                                      .premiumForHospitalization.cash
                                  }
                                  handleChange={(event) =>
                                    handleHospilization(event)
                                  }
                                  needValidation={true}
                                  logic={
                                    section9Recommend.product
                                      ...
                                      null ||
                                    section9Recommend.product
                                      .premiumForHospitalization.cash === 0
                                      ? false
                                      : true
                                  }
                                />
                            ...
                            <tr>
                              ...
                              <td className="px-2 py-2">
                                <Input
                                  ...
                                  value={
                                    section9Recommend.product
                                      .premiumForHospitalization.cpfMedisave
                                  }
                                  handleChange={(event) =>
                                    handleHospilization(event)
                                  }
                        ...
                        >
                          <div className="flex items-center justify-start gap-4 mb-5">
                            <Checkbox
                              label={benefit.title}
                              name="benefitData"
                              isChecked={checkBenefit(benefit.id)}
                              value={benefit.id}
                              onChange={(event) => handleBenefits(event, index)}
                            />
                          </div>
                          <div>
                            <p className="text-sm text-gray-light">
                      ...
                      >
                        <div className="flex items-center justify-start gap-4 mb-5">
                          <Checkbox
                            label={risk.title}
                            name="riskData"
                            isChecked={checkRisk(risk.id)}
                            value={risk.id}
                            onChange={(event) => handleRisks(event, index)}
                          />
                        </div>
                        <div>
                          <p className="text-sm text-gray-light">
                      ...
                      (value: any, index: any) => (
                        ...
                        >
                          <div className="flex mb-5">
                            <Checkbox
                              label={value.new_rider.riderName}
                              name="riderData"
                              isChecked={checkRiders(value.new_rider.id)}
                              value={value.new_rider.id}
                              onChange={(event) => setRiderAction(event, index)}
                            />
                          </div>
                          <div className="mb-5">
                            <p className="text-sm text-gray-light">
                              {value.new_rider.riderFeature}
                            </p>
                          </div>
                          {riderArr[value.new_rider.riderName] === true ? (
                            <div className="space-y-10">
                              ...
                              <RowSingleGrid>
                                ...
                                {riderBenefitValidation(value.new_rider.id) ==
                                false ? (
                                  <>
                                  ...
                                  value.new_rider.benefit.map(
                                    (valueBen: any, indexBen: any) => (
                                      ...
                                      >
                                        <div className="flex mb-5">
                                          <Checkbox
                                            label={valueBen.title}
                                            name="riderBenefitData"
                                            isChecked={checkRiderBenefit(
                                              valueBen.id,
                                              value.new_rider.id
                                            )}
                                            value={valueBen.id}
                                            onChange={(event) =>
                                              setRiderBenefitAction(
                                                event,
                                                value.new_rider.id,
                                                valueBen.id
                                              )
                              ...
                              <RowSingleGrid>
                                ...
                                {riderRiskValidation(value.new_rider.id) ==
                                false ? (
                                  <>
                                    <span className="w-full text-xs text-left text-red">
                                      Required Field
                                ...
                                {value.new_rider.risk?.length > 0 &&
                                  value.new_rider.risk.map(
                                    (valueRisk: any, indexRisk: any) => (
                                      <div
                                        className="w-full p-5 mb-5 border rounded-md border-gray-soft-strong"
                                        key={valueRisk.id}
                                      >
                                        <div className="flex mb-5">
                                          <Checkbox
                                            label={valueRisk.title}
                                            name="riderRiskData"
                                            isChecked={checkRiderRisk(
                                              valueRisk.id,
                                              value.new_rider.id
                                            )}
                                            value={valueRisk.id}
                    ...
                    <table className="w-full text-sm divide-y rounded-md divide-gray-soft-strong">
                      <thead className="bg-white-bone">
                        <tr className="border-b border-gray-soft-strong">
                          ...
                          <td>
                            <TextSmall>ALLOCATION(%)</TextSmall>
                          </td>
                        </tr>
                      </thead>
                      <tbody>
                        {section9Recommend?.product &&
                          section9Recommend.product.funds.map(
                            (dataPlatform: any, indexPlatform: any) => (
